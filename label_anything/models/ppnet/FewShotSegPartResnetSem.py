...
# @File    : FewShotSegPartResnetSem.py
...
class FewShotSegPart(nn.Module):
    ...
    def forward(self, supp_imgs, fore_mask, back_mask, qry_imgs):
        ...
        img_fts, feat_semantic = self.encoder(imgs_concat) #3*2048*53*53, 3*2048*53*53
        if self.training:
            output_semantic = self.aspp(feat_semantic) # 3*16*53*53

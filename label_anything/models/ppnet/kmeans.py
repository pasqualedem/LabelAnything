...
class KmeansClustering():
    ...
    def InitFunc(self, x):
        ...
        if self.init_mode == 'random':
            ...
            x1 = x.unsqueeze(1)
            x_lazy = torch.tensor(x1) ## N*1*D
            for c_id in range(self.num_cnt):
                init_lazy = torch.tensor(init_stat.unsqueeze(0)) ## 1*M*D
    ...
    def cluster(self, x, center=None):
        ...
        with torch.no_grad():
            x_lazy = torch.tensor(x.unsqueeze(1)) ## (N,1,D)
            cl = None
            for iter in range(self.iters):
                c_lazy = torch.tensor(center.unsqueeze(0)) ## 1*M*D

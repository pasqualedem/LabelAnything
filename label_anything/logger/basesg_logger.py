...
class BaseSGLogger(AbstractSGLogger):
    ...
    def _launch_tensorboard(self, port):
        self.tensor_board_process, _ = sg_trainer_utils.launch_tensorboard_process(
            self._local_dir, port=port
        )

    @multi_process_safe
    def _init_tensorboard(self, resumed, tb_files_user_prompt):
        self.tensorboard_writer = sg_trainer_utils.init_summary_writer(
            self._local_dir, resumed, tb_files_user_prompt
        )
    ...
    def _write_to_log_file(self, lines: list):
        with open(
            self.log_file_path, "a" if os.path.exists(self.log_file_path) else "w"
        ) as log_file:
            for line in lines:
                log_file.write(line + "\n")
    ...
    def add_video(
        ...
        if video.ndim < 5:
            video = video[None,]
        if self.use_tensorboard:
            self.tensorboard_writer.add_video(
                tag=tag, video=video, global_step=global_step
            )
    ...
    def add_histogram(
        ...
        if self.use_tensorboard:
            self.tensorboard_writer.add_histogram(
                tag=tag, values=values, bins=bins, global_step=global_step
            )

    @multi_process_safe
    def add_model_graph(
        self, tag: str, model: torch.nn.Module, dummy_input: torch.Tensor
    ):
        """
    ...
    def add_figure(self, tag: str, figure: plt.figure, global_step: int = None):
        ...
        if self.use_tensorboard:
            self.tensorboard_writer.add_figure(
                tag=tag, figure=figure, global_step=global_step
            )
    ...
    def add_file(self, file_name: str = None):
        if self.remote_storage_available and self.use_tensorboard:
            self.model_checkpoints_data_interface.save_remote_tensorboard_event_files(
                self.experiment_name, self._local_dir, file_name
            )
    ...
    def upload(self):
        if self.save_tensorboard_remote and self.use_tensorboard:
            self.model_checkpoints_data_interface.save_remote_tensorboard_event_files(
                self.experiment_name, self._local_dir
            )
        ...
        if self.save_logs_remote:
            ...
            self.model_checkpoints_data_interface.save_remote_checkpoints_file(
                self.experiment_name, self._local_dir, log_file_name
            )
    ...
    def close(self):
        ...
        if self.tensor_board_process is not None:
            try:
                ...
                logger.info(
                    "[CLEANUP] - Could not stop tensorboard process properly: "
                    + str(ex)
                )
...
class AbstractRunWrapper:
    ...
    def update_params(self, params):
        raise NotImplementedError(
            "update_params() not implemented in AbstractRunWrapper"
        )
    ...
    def get_local_checkpoint_path(self, phases):
        raise NotImplementedError(
            "get_local_checkpoint_path not implemented in AbstractRunWrapper"
        )

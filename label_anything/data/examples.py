...
def sample_over_inverse_frequency(frequencies):
    ...
    return torch.distributions.Categorical(probs=torch.tensor(list(probs.values()))).sample().item()


def generate_examples(query_image_id, sampled_classes, categories_to_imgs, min_size, sample_function, selection_strategy, num_examples):
    """
    ...
    Args:
        ...
        sampled_classes (list): list of classes to sample from
    ...
    for i in range(num_examples):
        found = False
        example_sampled_classes = sample_classes_from_query(torch.tensor(sampled_classes), min_size, sample_function)
        example_sampled_classes = [cat_id.item() for cat_id in example_sampled_classes]
        while not found:
            images_containing = get_image_ids_intersection(categories_to_imgs, example_sampled_classes, image_ids)

...
def collate_fn(batched_input):
    ...
    gts = torch.stack([collate_gts(x[BatchKeys.GROUND_TRUTHS], max_dims) for x in batched_input])
...
class DramTestDataset(LabelAnythingTestDataset):
    ...
    def _masks_to_tensor(self, masks, cat_ids):
        n = len(masks)
        c = len(cat_ids)
        tensor_shape = (n, c, self.prompt_processor.masks_side_length, self.prompt_processor.masks_side_length)
        ...
        for idx, img in enumerate(image_ids):
            ...
            for cat_id in cat_ids:
                if cat_id not in self.example_img2cat[img]:
                    continue
                masks[idx][cat_id].append(
                    (mask == cat_id).int().numpy()
                )
    ...
    def extract_prompts(self):
        ...
        images_sizes = [self._load_image(f"{self.example_image_dir}/{fname}.jpg") for fname in prompt_images_fnames]

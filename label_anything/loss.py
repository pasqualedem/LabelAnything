...
class AuxiliaryLoss(ComposedLoss):
    ...
    def __init__(
        self, task_loss_fn, aux_loss_fn, aux_loss_coeff: float = 0.2, **kwargs
    ):
        super().__init__()
    ...
    def forward(self, task_aux_output, target):
        ...
        if isinstance(
            task_loss, tuple
        ):  # SOME LOSS FUNCTIONS RETURNS LOSS AND LOG_ITEMS
        ...
        return (
            loss,
            torch.cat(
                (loss.unsqueeze(0), task_loss.unsqueeze(0), aux_loss.unsqueeze(0))
            ).detach(),
        )
...
class KDLogitsLoss(ComposedLoss):
    ...
    def __init__(
        self,
        task_loss_fn,
        distillation_loss_fn=VisklDivLoss(),
    ...
    def forward(self, kd_module_output, target):
        ...
        loss = (
            task_loss * (1 - self.distillation_loss_coeff)
...
class KDFeatureLogitsLoss(ComposedLoss):
    ...
    def __init__(
        self,
        task_loss_fn,
        feature_loss_fn=MSELoss(),
        distillation_loss_fn=VisklDivLoss(),
        dist_feats_loss_coeffs=(0.2, 0.4, 0.4),
        feats_loss_reduction="mean",
        **kwargs,
    ):
        super().__init__()
        self.task_loss_fn = task_loss_fn
    ...
    def forward(self, kd_output: FDOutput, target):
        ...
        feats_loss = self.feats_loss_reduction(
            torch.tensor(
                [
                    self.feature_loss_fn(student_feat, teacher_feat)
                    for student_feat, teacher_feat in zip(
                        kd_output.student_features, kd_output.teacher_features
                    )
                ],
        ...
        loss = (
            task_loss * self.dist_feats_loss_coeff[0]
...
class VariationalInformationLoss(nn.Module):
    def forward(self, student_feats, teacher_feats, **kwargs):
        ...
        return torch.sum(
            torch.log(sigma)
            + torch.div(torch.square(teacher_feats - mu), 2 * torch.square(sigma))
        )
